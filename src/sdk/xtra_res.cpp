...
wxObject *wxAuiToolBarXmlHandler::DoCreateResource()
{
    ...
    if (m_class == _T("tool"))
    {
        ...
        else
        {
            ...
            m_toolbar->AddTool(GetID(),
                               GetText(_T("label")),
                               GetCenteredBitmap(_T("bitmap"), wxART_OTHER, wxSize(16,16)),// bitmapSize));
                               GetCenteredBitmap(_T("bitmap2"), wxART_OTHER, wxSize(16,16)),//bitmapSize),
                               kind,
                               GetText(_T("tooltip")),
                               _T("Test hoor") /*GetText(_T("longhelp"))*/,
                               NULL);
    ...
    else /*<object class="wxAuiToolBar">*/
    {
        ...
        if(!toolbar && (m_class == _T("wxAuiToolBar")))
        {

            long style = GetStyle(_T("style"), wxNO_BORDER | wxAUI_TB_HORZ_LAYOUT);
            #ifdef __WXMSW__
            if (!(style & wxNO_BORDER)) style |= wxNO_BORDER;
            #endif

            style = wxAUI_TB_DEFAULT_STYLE | wxAUI_TB_OVERFLOW | wxAUI_TB_SPIN_TEXT;
           toolbar = new wxAuiToolBar(m_parentAsWindow, -1, wxDefaultPosition, wxDefaultSize, style);
        ...
        toolbar->SetArtProvider(new(wxAuiEmBlocksToolBarArt));
        toolbar->Realize();
        m_isInside = FALSE;
        m_toolbar = NULL;
        m_isAddon=false;
...
// Don't ask me why... >:-|
    ...
    #if wxCHECK_VERSION(2, 9, 0)
    bool istbar = node->GetAttribute(wxT("class"), wxEmptyString).Matches(_T("wxAuiToolBar"));
    bool istool = node->GetAttribute(wxT("class"), wxEmptyString).Matches(_T("tool"));
    bool issep = node->GetAttribute(wxT("class"), wxEmptyString).Matches(_T("separator"));
    #else
    wxString tmp = node->GetPropVal(wxT("class"), wxEmptyString);
    bool _istbar = node->GetPropVal(wxT("class"), wxEmptyString).Matches(_T("wxAuiToolBar"));
